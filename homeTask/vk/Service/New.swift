...
class VkApiNewItem: Object, Decodable {
    ...
    @objc dynamic var photoImageURL: String?
    ...
    convenience required init(from decoder: Decoder) throws {
        ...
        for _ in 0..<(attachments?.count ?? 0){
            ...
            case "photo":
                ...
                self.photoImageURL = try? photo?.decode(String.self, forKey: .photo_604)
                break
            case "video":
                let video = try? firstAttachmentsValues?.nestedContainer(keyedBy: VideoKeys.self, forKey: .video)
                self.photoImageURL = try? video?.decode(String.self, forKey: .photo_800)
                break
            default: break
            }
        }
        ...
        for _ in 0..<(items?.count ?? 0){
            ...
            self.photoImageURL = try? firstItemsValues?.decode(String.self, forKey: .photo_604)
        }
    }
}
